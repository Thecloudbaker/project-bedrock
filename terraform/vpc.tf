...
resource "aws_eip" "nat" {
  ...
  tags       = { Name = "project-bedrock-nat-eip-${count.index + 1}" }
}
...
resource "aws_nat_gateway" "main" {
  ...
  depends_on    = [aws_internet_gateway.main]
  tags          = { Name = "project-bedrock-nat-${count.index + 1}" }
}
...
resource "aws_route_table" "private" {
  ...
  tags = { Name = "project-bedrock-private-rt-${count.index + 1}" }
}
